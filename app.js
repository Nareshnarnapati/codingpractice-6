...
app.get("/districts/:districtId/", async (request, response) => {
  ...
  const districtArray = await db.run(districtDetails);
...
app.put("/districts/:districtId", async (request, response) => {
  ...
  const updateDistrictDetails = `UPDATE district SET  district_name='${districtName}',
                                                      state_id= '${stateId}',
                                                      cases = '${cases}',
                                                      cured = '${cured}',
                                                      active = '${active}',
...
app.get("/districts/:districtId/details/", async (request, response) => {
  const { districtId } = request.params;
  const stateQuery = `SELECT state_name
                      FROM state 
                          NATURAL JOIN district
                      WHERE district_id = ${districtId}`;

  const stateName = await db.get(stateQuery);
  response.send(convertDbObjectToResponseObject(stateQuery));
});
module.exports = app;
